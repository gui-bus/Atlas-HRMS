...
export default function RejectLeavePage() {
  ...
  const { data: leave, isLoading } = useQuery({
    queryKey: ["leave", id],
    queryFn: () => vacationService.getLeaves().then((list) => list.find((l) => l.id === id)),
    enabled: !!id,
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: RejectFormValues) => {
      return vacationService.updateLeaveStatus(id, "REJECTED", data.reason);
    },
    onSuccess: () => {
      router.push(`/${locale}/absences/leaves`);
    },
  });
    ...
    <RbacGuard allowedRoles={["ADMIN", "HR", "MANAGER"]}>
      <div className="p-6 md:p-8 space-y-6 w-full animate-fade-in">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Rejeitar Solicitação de Licença</h1>
            <p className="text-muted-foreground text-sm">
              Explique o motivo de indeferimento da licença ou afastamento.
            </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 w-full">
          <div className="space-y-4">
            <FormSectionHeader
              ...
              description="Aponte de forma clara e formal o motivo para a rejeição da licença."
              icon={XCircle}
            />
            ...
            <div className="space-y-2 max-w-xl">
              ...
              <Input
                id="reason"
                placeholder="Ex: Documento de comprovante de atestado ilegível..."
                {...register("reason")}
              />
