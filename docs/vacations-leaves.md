...
2. **Tipos de Licença**: Classificadas sob o enum `LeaveType` (`MEDICAL`, `PARENTAL`, `LEGAL`, `UNPAID`, `OTHER`). Quando o tipo selecionado for `OTHER` (Outros Afastamentos), a API e o banco de dados registram e exibem o tipo textual livre no campo `customType`.
